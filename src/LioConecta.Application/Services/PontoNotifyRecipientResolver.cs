...
public sealed class PontoNotifyRecipientResolver(
    ...
    IAppSettingsProvider settingsProvider)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var roles = ParseStringList(settingsProvider.GetString(AppSettingKeys.PontoNotifyRoles, "[\"HR\"]"));
        ...
        var notifyEmails = ParseStringList(settingsProvider.GetString(AppSettingKeys.PontoNotifyEmails, "[]"));
    ...
    public async Task<(bool CanAccess, bool IsRhScope)> CanManageAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (await permissionService.HasPermissionAsync("ponto.manage", DataScope.Global, cancellationToken))
        {
            return (true, true);
        }

        if (await permissionService.HasPermissionAsync("ponto.approve", DataScope.Global, cancellationToken))
        {
            return (true, true);
        }

        if (await permissionService.HasPermissionAsync("ponto.approve", DataScope.Team, cancellationToken))
        {
