    ...
    IAppSettingsProvider settingsProvider)
{
    ...
    public async Task<(bool CanAccess, bool IsRhScope)> CanManageAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (currentRoles.Contains(UserRole.Admin) || currentRoles.Contains(UserRole.HR))
        {
            return (true, true);
        }

        var allowedEmails = ParseStringList(settingsProvider.GetString(AppSettingKeys.LeaveNotifyEmails, "[]"));
        var current = await personRepository.GetByIdAsync(currentPersonId, cancellationToken);
        if (current is not null
            && allowedEmails.Any(e => string.Equals(e, current.Email, StringComparison.OrdinalIgnoreCase)))
        {
            return (true, true);
        }
