...
public sealed class AuthService(
    ...
    RbacTokenResolver rbacTokenResolver,
...

{
...

    {
        ...
        var testUser = await db.TestUsers

            .FirstOrDefaultAsync(u => u.IsActive && u.Email.ToLower() == email, cancellationToken);
...

        {
            ...
            if (testUser.ExpiresAt.HasValue && testUser.ExpiresAt.Value < DateTimeOffset.UtcNow)

            {

                throw new UnauthorizedAccessException("Conta de teste expirada.");
            ...
            var person = await ResolveTestUserPersonAsync(testUser, cancellationToken);

            var permissions = await rbacTokenResolver.ResolvePermissionsAsync(

                RbacSubjectType.TestUser, testUser.Id, cancellationToken);

            var testUserRoles = await ResolveLegacyRolesFromAssignmentsAsync(RbacSubjectType.TestUser, testUser.Id, cancellationToken);

            var (token, expiresInSeconds) = portalJwtService.CreateToken(

                person, testUserRoles, RbacSubjectType.TestUser, testUser.Id, testUser.SecurityStamp, true, permissions);

            await RecordLoginSucceededAsync(testUser.Email, cancellationToken);

            var me = PersonMapper.ToMe(person, testUserRoles, permissions, "test", true);

            logger.LogInformation("Login test user bem-sucedido para {Email}.", email);

            return new LoginResponse(token, expiresInSeconds, me);
...

        {
...

            {
                ...
                ldapPerson = portalUser.Person;

                roles = await ResolveLegacyRolesFromAssignmentsAsync(RbacSubjectType.PortalUser, portalUser.Id, cancellationToken);
                ...
                securityStamp = portalUser.SecurityStamp;
...

            {
                ...
                ldapPerson = portalUser.Person ?? await ResolveOrCreatePersonAsync(ldapResult, cancellationToken);
                ...
                logger.LogInformation("Login LDAP bem-sucedido para {Email} (conta local também cadastrada).", email);
...

        {
            ...
            subjectId = ldapPerson.Id;

            securityStamp = ldapPerson.Id.ToString("N");
        ...
        var userPermissions = await rbacTokenResolver.ResolvePermissionsAsync(subjectType, subjectId, cancellationToken);

        var (jwt, exp) = portalJwtService.CreateToken(

            ldapPerson, roles, subjectType, subjectId, securityStamp, false, userPermissions);

        await RecordLoginSucceededAsync(ldapPerson.Email, cancellationToken);

        return new LoginResponse(jwt, exp, PersonMapper.ToMe(ldapPerson, roles, userPermissions, subjectType.ToString().ToLowerInvariant(), false));
...

    {
        ...
        var slugs = await db.SubjectRoleAssignments.AsNoTracking()

            .Where(a => a.SubjectType == subjectType && a.SubjectId == subjectId)

            .Join(db.Roles.AsNoTracking(), a => a.RoleId, r => r.Id, (_, r) => r.Slug)
...

        {
            ...
            if (Enum.TryParse<UserRole>(slug, true, out var role))
        ...
        return roles.Distinct().DefaultIfEmpty(UserRole.Employee).ToList();
