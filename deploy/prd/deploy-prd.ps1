...
# Deploy PRD — mesma stack/portas do DEV/HML, host 10.0.0.88
...
$ConfigFile = Join-Path $DeployConfigDir "prd.local.env"
...
$staging = Join-Path $env:TEMP "lioconecta-prd-deploy-$(Get-Date -Format yyyyMMddHHmmss)"
New-Item -ItemType Directory -Path $staging -Force | Out-Null
Write-Host "Staging: $staging"
Write-Host "Building frontend (PRD)..."
...
$env:VITE_COMPASS_APP_URL = "http://10.0.0.88:8094"
$env:VITE_UNILIO_APP_URL = "http://10.0.0.88:8096"
...
Write-Host "Syncing to PRD ($hostName)..."
...
Write-Host "Starting containers (postgres/redis -> api -> nginx)..."
# Write remote script with LF endings to avoid bash `\r` errors from PowerShell here-strings.
$bootScript = @"
#!/usr/bin/env bash
set -euo pipefail
cd '$remoteDir/current'
export POSTGRES_PASSWORD='$pgPass'
export LIOSNECTA_HTTP_PORT='$httpPort'
docker compose -f docker-compose.dev.yml --env-file .env up -d --build postgres redis
sleep 8
docker compose -f docker-compose.dev.yml --env-file .env up -d --build api
docker compose -f docker-compose.dev.yml --env-file .env up -d nginx
"@
$bootLocal = Join-Path $env:TEMP "lioconecta-prd-boot.sh"
[IO.File]::WriteAllText($bootLocal, ($bootScript -replace "`r`n", "`n" -replace "`r", "`n"))
scp @sshArgs $bootLocal "${sshTarget}:/tmp/lioconecta-prd-boot.sh"
ssh @sshArgs $sshTarget "bash /tmp/lioconecta-prd-boot.sh"
$baseUrl = "http://${hostName}:$httpPort"
Write-Host "Waiting for API boot (migrations+seed)..."
$booted = $false
for ($i = 0; $i -lt 48; $i++) {
  Start-Sleep -Seconds 5
  try {
    $homeResp = Invoke-WebRequest -Uri "$baseUrl/" -UseBasicParsing -TimeoutSec 8
    if ($homeResp.StatusCode -eq 200) {
      # Probe health even if 503 (redis not seeded yet)
      try { Invoke-WebRequest -Uri "$baseUrl/health" -UseBasicParsing -TimeoutSec 8 | Out-Null; $booted = $true; break }
      catch {
        if ($_.Exception.Response -and [int]$_.Exception.Response.StatusCode -ge 500) { $booted = $true; break }
      }
    }
  } catch {}
  Write-Host "  ... aguardando boot ($($i+1)/48)"
}
if (-not $booted) {
...
Write-Host "Seeding redis/cors for PRD..."
ssh @sshArgs $sshTarget "cd $remoteDir/current; LIOSNECTA_HTTP_PORT=$httpPort LIOSNECTA_PUBLIC_HOST=$publicHost bash seed-dev-settings.sh; bash scripts/apply-db-fixes.sh; chmod +x scripts/seed-dev-glpi-settings.sh; bash scripts/seed-dev-glpi-settings.sh"
ssh @sshArgs $sshTarget "cd $remoteDir/current; docker compose -f docker-compose.dev.yml restart api nginx"
...
for ($i = 0; $i -lt 40; $i++) {
  ...
  Write-Host "  ... aguardando ready ($($i+1)/40)"
}
if (-not $ready) {
  ssh @sshArgs $sshTarget "cd $remoteDir/current; docker compose -f docker-compose.dev.yml logs api --tail=100"
...
ssh @sshArgs $sshTarget "cd $remoteDir/current; docker compose -f docker-compose.dev.yml restart nginx"
...
$evidenceDir = Join-Path $backRoot "deploy\prd\evidence"
